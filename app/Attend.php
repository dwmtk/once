...
class Attend extends Model
{
    ...
    public static function getAttendEvents($user_id){
        // 特定のユーザが参加しているイベント一覧を取得
        // 注意：辞退済みも含む
        return \DB::select('select * from events inner join attends on events.id = attends.event_id where attends.user_id = ?', [$user_id]);
    }
    public static function getAttendUsers($event_id){
        // 特定のイベントに参加しているユーザ一覧を取得
